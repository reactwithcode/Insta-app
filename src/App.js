...
import instaappPic from './assets/img/instaapp.svg'
import addPostPic from './assets/img/add-post.svg'
import avatarPic from './assets/img/user.svg'
...
function App() {
  ...
  // This is to toggle from EN to IND
  const toggleLang = () => setLang(!lang);


  // The below is what checks if you are logged in or not, and keeps you logged in on refresh
  useEffect(() => {
    // 4a listen every single time authUser is changed, authUser will be called
    // onAuthStateChanged(authUser) use cookies tracking, so data won't disappear
  ...
  // 3b puling data from database
  useEffect(() => {
      // This is where the code runs
      db.collection('posts').orderBy('timestamp', 'desc').onSnapshot(snapshot => {
        // every time a new post/documents data is added on posts collection, this code fires up and maping documents on posts collection
        setPosts(snapshot.docs.map(doc => ({
          // 3c
          id: doc.id, // get unique id per document
          post: doc.data() // get all the propeties on document such as caption, username
        })));
      })
  }, []);
 ...
 */
  ...
  // 3d Firebase user authentication
  // config authentication on firebase
  const signUp = (event) => {

    // This is to prevent the page from refreshing when we submit the form
    event.preventDefault();
    // 4 create new user dan post data email and email from state to firebase authentication database
    auth.createUserWithEmailAndPassword(email, password)
    .then((authUser) => {
      // 4b update data profile of authUser and displayName to attribute documenet
      ...
      >
        <div style={modalStyle} className={classes.paper}>
          <form className="app__signup">
            ...
            <Input 
              type="text"
              placeholder={lang ? "username":"nama akun"}
              value={username}
              onChange={(e) => setUsername(e.target.value)}
            /> 
            <Input 
              placeholder={lang ? "email":"alamat email"}
              type="text"
              value={email}
            ...
            />
            <Button type="submit" onClick={signUp}>{lang ? "Sign Up":"Daftar"}</Button>
        ...
        <div style={modalStyle} className={classes.paper}>
          <form className="app__signup">
            ...
            <Input 
              ...
              onChange={(e) => setPassword(e.target.value)}
            />
            <Button type="submit" onClick={signIn}>{lang ? "Sign In":"Masuk"}</Button>
      ...
      <header className="app__header">
        ...
        <div className="app__loginContainer">
              ...
              <div className="loginLeft">
                { user ? (
                  <Button onClick={() => auth.signOut()}>{lang ? "Logout":"Keluar"}</Button>
                ) : (
                  <Button onClick={() => toggleLang()}>{lang ? "EN":"IND"}</Button>
                )
                }
            ...
            <div className="loginRight">
              <Button  aria-controls="fade-menu" aria-haspopup="true" > 
                <MenuPopupState 
                  ...
                  labeltopass={lang ? "Versi Bahasa Indonesia" : "English Version"}
                  signout={() => auth.signOut()}
                  signoutlabel={lang ? "Logout":"Keluar"}
                  signin={() => setOpenSignIn(true)}
                  signinlabel={lang ? "Sign In" : "Masuk"}
                  signup={() => setOpen(true)}
                  signuplabel={lang ? "Sign Up" : "Daftar"}
                />
              </Button>
          ...
          <div>
            ...
            <div className="footer__icons">
              <div className="footer__left">
                <img onClick={home} className="app__home" src={homePic} alt='home icon to go back up'/>         
              </div>

              <div className="footer__middle">
                <img onClick={() => setOpenImageUpload(true)} className="app__add-postImg" src={addPostPic} alt='plus icon to add posts'/>
              </div>
